...
def problem0c(circle, n, window):
    ...
    window.render()
    new_center_x = circle.center.x + (2 * circle.radius)
    new_center_y = circle.center.y
    for k in range (n):
        new_center = rg.Point(new_center_x, new_center_y)
        circle_new = rg.Circle(new_center,circle.radius)
        circle_new.attach_to(window)
        window.render(0.5)
        new_center_x = new_center_x + 2*circle.radius
